...
# Get row index and user record by email
...
# Increment usage count if within limit
...
# Add new user with OTP verification info
...
def verify_otp(email, entered_otp):
    ...
    if user and str(user.get("otp")).strip() == str(entered_otp).strip():
        get_sheet().update_cell(row, 9, "yes")  # Column 9: verified
        return True
    return False
